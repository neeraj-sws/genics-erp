...
<form  id="update_user" method="post" onsubmit="delivery_boy_update();return false;">
     ...
     <option  <?php if($uinfo->status==0){ echo 'selected'; }else{ echo ''; } ?> value="0">Inactive</option>
					...
					<div class="col-md-6">
						<div class="form-group SelectFullWidth">
							...
							<select class="form-control " name="admin" id="role_id">
								<option value="">--Select Admin--</option>
								<option <?php if ($uinfo->is_admin == 1) {
											echo 'selected';
										} else {
											echo '';
										} ?> value="1">Yes</option>
								<option <?php if ($uinfo->is_admin == 0) {
											echo 'selected';
										} else {
											echo '';
										} ?> value="0">No</option>
...
<script type="text/javascript">
  ...
  function upload_photo(id)
	...
	function delivery_boy_update()
	{
	
		$('#update_user .st_loader').show();
		$.ajax({  
		  url :BASE_URL+"admin/delivery_boy/delivery_boy_update",  
		  method:"POST",  
		  dataType:"json",  
		  ...
		  success:function(res){  
			if(res.status == 0){
			  ...
			  toastr.success('Delivery boy Updated Successfully','Success');
