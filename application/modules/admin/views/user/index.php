        ...
        <div class="col-12">
            ...
            <!-- /.card-header -->
				<div class="card-body pt-3 pb-3 ">
					<div class="row well ">
						<div class="col-md-2 ml-auto"><button type="button" class="btn btn-primary btn-block float-right" onclick="add_user()"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;Add</button></div>
					</div>                
				</div>                
            </div>  
			<div class="card">
...
$(document).ready(function(){
    ...
    window.table = $('#datatable').DataTable({
		...
		"pageLength": 10,
		"ajax": {
            "url": "<?php echo base_url();?>admin/user/user_ajax_list",
            "type": "POST",
            "dataType": "json",
            "data": function(data){
   ...
   function add_user(){
	...
	function user_save(){
		$('#add_user .st_loader').show();
		 $.ajax({  
		   url :BASE_URL+"admin/user/user_save",  
		   method:"POST",  
		   dataType:"json",  
	...
	function user_edit(id){
		$.ajax({
		   url :"<?php echo base_url();?>admin/user/user_edit",
		   method:"POST",
		   data:{id:id},
		   success:function(res){
			$('#modal-default .modal-content').html(res);
	...
	function user_update(){
		$('#update_user .st_loader').show();
		 $.ajax({  
		   url :BASE_URL+"admin/user/user_update",  
		   method:"POST",  
		   dataType:"json",  
	...
	function user_view(id){
		$.ajax({
		   url :"<?php echo base_url();?>admin/user/user_view",
		   method:"POST",
		   data:{id:id},
		   success:function(res){
			$('#user_view').html(res);
    ...
    if(confirm("Are you sure, You want to delete selected User ?")){
      $.ajax({
         url :"<?php echo base_url();?>admin/user/user_multiple_delete",
         method:"POST",
         dataType:"json",
  ...
  function user_status(id,type){
	  ...
	  if(type == 0){
		...
		$.ajax({
		   url :"<?php echo base_url();?>admin/user/user_status",
		   method:"POST",
		   dataType:"json",
	  ...
	  if(type == 0){
		...
		if(confirm("Are you sure, You want to "+status+" this User ?")){
		$.ajax({
		   url :"<?php echo base_url();?>admin/user/user_verify",
		   method:"POST",
		   dataType:"json",
		   data:{id:id,type:type},
		   success:function(res){
	...
	function kyc_status(id,type){
	  ...
	  if(type == 0){
		...
		$.ajax({
		   url :"<?php echo base_url();?>admin/user/kyc_status",
		   method:"POST",
		   dataType:"json",
		   data:{id:id,type:type},
		   success:function(res){
	...
	function kyc_view(id){
		$.ajax({
		   url :"<?php echo base_url();?>admin/user/kyc_view",
		   method:"POST",
		   data:{id:id},
		   success:function(res){
			$('#modal-default .modal-content').html(res);
