...
class Admin extends Base_Controller {
	...
	public function  order_ajax_list(){ 
		...
		$where_condition1 = array('orders.is_cancel !='=>1);
		$where_condition2  = array('orders.is_dispatch !='=>1);
		...
		if($lists){
			foreach ($lists as $list) {
				...
				$row[] = '<td><input class="form-check-input me-3 filed_check d-none"  name="check[]"  type="checkbox"  value="' . $list->id . '"></td><a class="text-dark" href="admin/detail/'.$list->id.'">'.$no.'</a>';
	...
	$columns="orders.*,distributor.full_name as distributorName,delivery_boy.full_name as deliveryBoyName,upload_images.file,distributor_other.full_name as distributor_other_name,distributor_third_party.full_name as distributor_third_party_name,distributor_attachment.file as distributor_attachment_file";
		...
		$where_condition = array('order_activity.order_id'=>$this->uri->segment(3));
		$columns2="order_activity.*,activity_type.message_type,activity_type.id as message_id,users.full_name,distributor.full_name as distributor_name,delivery_boy.full_name as deliveryboy_name";
		$joins2[] = array('table'=>'activity_type','condition'=>'activity_type.id=order_activity.message_type','jointype'=>'left');
		$joins2[] = array('table'=>'users','condition'=>'users.id=order_activity.user_id','jointype'=>'left');
		$joins2[] = array('table'=>'distributor','condition'=>'distributor.id=order_activity.distributor_id','jointype'=>'left');
		$joins2[] = array('table'=>'delivery_boy','condition'=>'delivery_boy.id=order_activity.deliveryboy_id','jointype'=>'left');
		$data['orderActivitys'] = $orderActivitys = $this->base_model->select_join_result('order_activity',$where_condition,$joins2,$columns2);
		
	//	echo '<pre>';print_R($data['distributorAttachment']);die;
		...
		$data['distributors'] = $this->base_model->select_data('distributor',array('category' => $category->id , 'status'=>1));	
	...
	public function updateToken(){
		...
		if($this->form_validation->run() == FALSE){
			...
			$uinfo = $this->base_model->select_row('users',array('role'=>1,'id'=>$this->session->userdata('id')));

			if(!$uinfo){
				$res =array('status'=>2,'msg'=>'User Not Found');
				echo json_encode($res);die;
			}
			else{
			$udata =array(
				'fcm_web_token'=>trim($this->input->post('token')),
			);
			$this->base_model->update_data('users',array('id'=>$this->session->userdata('id')),$udata);
           
             //for testing of web notification against token
             
                // $this->load->model('FcmModel');
                // $fcmTitle = "Token created!";
                // $fcmBody = "You have a new Token created.!";
                // $fcmLink = "admin/order";
              
        
                // $this->FcmModel->sendNotifictaion($fcmTitle,$fcmBody,[trim($this->input->post('token'))],$fcmLink);
