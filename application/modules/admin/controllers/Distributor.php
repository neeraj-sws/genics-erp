...
class Distributor extends Base_Controller {
	...
	public function user_ajax_list(){ 
		...
		$coloum_search = array('distributor.full_name','distributor.email','distributor.phone','category.title');
	...
	public function user_save()
	{
		...
		$this->form_validation->set_rules('phone', 'Phone Number', 'required|exact_length[10]|numeric|is_unique[distributor.phone]');
		...
		if($this->form_validation->run() == FALSE){
			...
			if($admin == 2){
				$selected_admins = $this->input->post('selected_admin');
				$selected_admin = implode(",",$selected_admins);
				}else{
					$selected_admin = 0;
				}
	...
	public function user_edit(){
		...
		$data['roles'] = $this->base_model->select_data('all_cities',array());
	   $data['categorys'] = $this->base_model->select_data('category',array());
	   $where1 = array('distributor.id !='=>$this->input->post('id'));
	   $data['distributors'] = $this->base_model->select_data('distributor', $where1);
	...
	function check_for_phone($phone){
		...
		if (!empty($check)){
			$this->form_validation->set_message('check_for_phone','Phone Field should contain unique value12222.');
			return FALSE;
		}
		return TRUE;
	}
