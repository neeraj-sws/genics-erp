...
class Delivery_boy extends Base_Controller { 
    ...
    public function  delivery_boy_ajax_list(){ 
		...
		$coloum_search = array('delivery_boy.full_name','delivery_boy.phone','delivery_boy.email');
 ...
 public function delivery_boy_save()
	{
		...
		$this->form_validation->set_rules('phone', 'Phone Number', 'required|exact_length[10]|numeric|is_unique[delivery_boy.phone]');
...
function check_for_user($email){
	$check = $this->base_model->select_row('delivery_boy',array('email'=>$email));
	if (!empty($check)){
...
function check_for_phone($phone){
	$check = $this->base_model->select_row('delivery_boy',array('phone'=>$phone));
	if (!empty($check)){
