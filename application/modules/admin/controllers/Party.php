...
class Party extends Base_Controller {
    ...
    public function index()
	{
		...
		$data['sub_nav'] ='party';
		$data['title'] ='party';
		$this->loadAdminTemplate('party/index',$data);
	}
public function  party_ajax_list()
{ 
		...
		$coloum_search = array('users.full_name','users.phone','distributor.full_name');
	...
	$columns = 'users.*,orders.user_id,orders.code,orders.amount,distributor.full_name as distributor_name,distributor.id as distributors_id';
	$joins[] = array('table'=>' orders','condition'=>'orders.user_id=users.id','jointype'=>'left');
	$joins[] = array('table'=>' distributor','condition'=>'distributor.id=orders.distributor_id','jointype'=>'left');
		...
		if($lists){
			foreach ($lists as $list) {
				...
				$row[] =  $list->full_name;
				$row[] =  $list->phone;
				// $part_distributor = $this->base_model->select_row('party_distributors',array('party_distributors.party_id'=>$list->id));
				// if($part_distributor){
				$distributor = $this->base_model->select_row('distributor',array('distributor.id'=>$list->distributor_id));
				// }else{
				// 	$distributor = '';
				// }
				if($distributor){
					$add_distributor = $distributor->full_name . '<a class="text-muted " onclick="change_distributor('.$list->id.')">' ." <i class='fa fa-edit'></i>".'</a>';
				}else{
					$add_distributor ='<button type="button" class="btn btn-success btn-xs" onclick="add_distributor('.$list->id.')"><small>Add</small></button>';
				}
			
				$row[] = $add_distributor;
				if($list->amount){
				$row[] ='<span class="badge badge-dark" onclick="view_order('.$list->id.')">'. "₹". $list->amount.'</span>';
				}else{
				   $row[] ='-'; 
				}
...
public function distributor_save()
{
	...
	if($this->form_validation->run() == FALSE){
		...
		$distributer = $this->base_model->select_row('distributor',array('id'=>$this->input->post('distridutor')));

		$category = $this->base_model->select_row('category',array('id'=>$distributer->category));
		if($category->title == 'third_party'){ 
			$m = json_encode(array('This Distributor category is third party'));
			$res =array('status'=>0,'msg'=>$m);
			echo json_encode($res);die;
		}else{		
		$data =array(
        ...
        $distributor_id = array('distributor_id'=>$this->input->post('distridutor'));
		$distributer_id_update = $this->base_model->update_data('orders',array('user_id'=>$this->input->post('party')),$distributor_id);
