...
class Api_model extends CI_Model{
	...
	public function getSquad($match_data){
		...
		$teams =array();
		$teams['a']['name']=$match_data->team_a;
		$teams['b']['name']=$match_data->team_b;
		...
		foreach($match_Data as $match){
			...
			if($match->team_type == 'a'){
				...
				$teams['a']['playes'][]=$palyer;
			}

			if($match->team_type == 'b'){
				$teams['b']['playes'][]=$palyer;
			}
		}
		return $teams;
	}
	...
	public function  getMatchScoreBord($match_id)
	{
		...
		foreach($match_data as $match){
			...
			if($match->team_type == 'a'){ 
				...
				if(isset($res->score->$index->batting->score)){ 
					$team_a['batting'][$a_bat]=$res->score->$index->batting->score;
					$team_a['batting'][$a_bat]->player=$match->player_name;
					$a_bat++;
				}

				if(isset($res->score->$index->batting->dismissal)){ 
					$team_a['wickets'][$a_wkt]=$res->score->$index->batting->dismissal;
					$team_a['wickets'][$a_wkt]->overs=implode('.',$res->score->$index->batting->dismissal->overs);
					$team_a['wickets'][$a_wkt]->player=$match->player_name;
					$a_wkt++;
				}

				if(isset($res->score->$index->bowling->score)){ 
					$team_a['bowling'][$a_bol]=$res->score->$index->bowling->score;
					$team_a['bowling'][$a_bol]->player=$match->player_name;
					$a_bol++;
				}

				if(isset($res->score->$index->fielding)){ 
					$team_a['fielding'][$a_field] =$res->score->$index->fielding;
			...
			if($match->team_type == 'b'){ 
				...
				if(isset($res->score->$index->batting->score)){ 
					$team_b['batting'][$b_bat]=$res->score->$index->batting->score;
					$team_b['batting'][$b_bat]->player=$match->player_name;
					$b_bat++;
				}

				if(isset($res->score->$index->batting->dismissal)){ 
					$team_b['wickets'][$b_wkt]=$res->score->$index->batting->dismissal;
					$team_b['wickets'][$b_wkt]->overs=implode('.',$res->score->$index->batting->dismissal->overs);
					$team_b['wickets'][$b_wkt]->player=$match->player_name;
					$b_wkt++;
				}

				if(isset($res->score->$index->bowling->score)){ 
					$team_b['bowling'][$b_bol]=$res->score->$index->bowling->score;
					$team_b['bowling'][$b_bol]->player=$match->player_name;
					$b_bol++;
				}

				if(isset($res->score->$index->fielding)){ 
					$team_b['fielding'][$b_field] =$res->score->$index->fielding;
