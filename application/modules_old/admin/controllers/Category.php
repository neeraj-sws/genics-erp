...
class Category extends Base_Controller {
    ...
    public function index()
	{
		...
		$data['title'] ='category';
		$this->loadAdminTemplate('category/index',$data);
	}
    public function category_ajax_list(){ 
		...
		if(isset($_POST['searchName']) && !empty($_POST['searchName'])){
			$where_like = array('category.title'=>$_POST['searchName']);
		}
		
		if(isset($_POST['status']) && $_POST['status'] !=''){
			$where_condition2 = array('category.status'=>$_POST['status']);
		}
		$where_condition1 = array();
		$where = array_merge($where_condition1,$where_condition2);
		$coloum_search = array('category.title');
		$order_by = array('category.id'=>'DESC');
		
		if(isset($requestData['order'])){
			if($requestData['order']['0']['column'] == 1){
				$order_by = array('category.title'=>$requestData['order']['0']['dir']);
			}
		}
		
		$columns="category.*";
		$joins = array();
		$lists = $this->base_model->get_where_in_lists('category',$coloum_search,$order_by,'',$where_in,$joins,$columns,'category.id',$where,'',$where_like);
		...
		if($lists){
			foreach ($lists as $list) { 
				...
				if($list->status == 0){
					$status = '<button type="button" class="btn btn-danger btn-xs" onclick="category_status('.$list->id.',1)"><small>Inactive</small></button>';
				}else{
					$status = '<button type="button" class="btn btn-success btn-xs" onclick="category_status('.$list->id.',0)"><small>Active</small></button>';	
				}
				$row[] = $status;
	
				
				$orders='<button type="button" class="btn btn-info btn-xs"><i class="fa fa-eye" aria-hidden="true"></i></button>';
				$buttons = '<button type="button" class="btn btn-success btn-xs" onclick="category_edit('.$list->id.')"><small>Edit</small></button>';
				//$buttons .= '&nbsp;&nbsp;<button type="button" class="btn btn-success btn-xs" onclick="user_view('.$list->id.')"><small>View</small></button>';
				$buttons .= '&nbsp;&nbsp;<button type="button" class="btn btn-danger btn-xs" onclick="category_delete('.$list->id.')"><small>Delete</small></button>';
				$row[] = $buttons;
				$row['id'] = $list->id;
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->base_model->count_all_where_in('category',$where_in,'category.id',$where,$joins),
			"recordsFiltered" =>$this->base_model->count_filtered_where_in('category',$coloum_search,$order_by,'',$where_in,$joins,$columns,'category.id',$where),
			"data" => $data,
		);
	...
	public function category_save()
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			$uid = $this->base_model->insert_data('category',$data);
			$res =array('status'=>1);
			echo json_encode($res);die;
	...
	public function category_status(){
		$this->base_model->update_data('category',array('id'=>$this->input->post('id')),array('status'=>$this->input->post('type')));
		$res =array('status'=>1);
		echo json_encode($res);die;
	}	
	public function category_edit(){

		$where = array('category.id'=>$this->input->post('id'));
		$columns="category.*";
		$joins = array();
		$data['cinfo'] = $this->base_model->select_join_row('category',$where,$joins,$columns);
	...
	{
		$original_value = $this->base_model->select_row('category',array('id'=>$this->input->post('id')));
		$this->form_validation->set_rules('title', 'title', 'required');	 
		$this->form_validation->set_rules('status', 'Status', 'required');
		...
		if($this->form_validation->run() == FALSE){
			...
			$this->base_model->update_data('category',array('id'=>$this->input->post('id')),$data);
			$res =array('status'=>1);
			echo json_encode($res);die;
		}
	}	
	public function category_delete(){
		$this->base_model->delete_data('category',array('id'=>$this->input->post('id')));
		$res =array('status'=>1);
		echo json_encode($res);die;
	}
