      ...
      <h4 class="modal-title">Edit Party Distributor</h4>
      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
       <span aria-hidden="true">&times;</span>
     </button>
   </div>
   
   <form  id="update_user" method="post" onsubmit="distributor_update();return false;">
				...
				<div class="form-group SelectFullWidth">
					...
					<select class="form-control " name="distridutor" id="role_id">
						...
						<?php foreach($distributors as $distributor){?>
							<option <?php if($uinfo->distributor_id == $distributor->id){ echo 'selected'; } ?> value="<?php echo $distributor->id;?>"><?php echo $distributor->full_name;?></option>       
						<?php } ?>
					</select>
  ...
  $(document).ready(function() {
   ...
   $('.allowno').on('paste',function (e){
	...
	function distributor_update()
	{
		...
		$.ajax({  
		  url :BASE_URL+"admin/party/distributor_update",  
		  method:"POST",  
		  dataType:"json",  
		  ...
		  success:function(res){  
			if(res.status == 0){
			  ...
			  toastr.success('Distributor Updated Successfully','Success');
			  $('#modal-default').modal('hide');
			  $('#modal-default .modal-content').html('');
			  table.draw( false );
			}
